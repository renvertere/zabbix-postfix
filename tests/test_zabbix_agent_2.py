...
def zabbix_agent2_output():
    ...
    results = {}
    for key in keys:
        output = subprocess.check_output(["zabbix_get", "-s", "127.0.0.1", "-p", "10052", "-k", key]).decode("utf-8").strip()
        results[key] = output
    return results

def test_zabbix_agent2(zabbix_agent2_output):
    for key, output in zabbix_agent2_output.items():
        ...
        elif key in ["net.tcp.port[,25]", "mailq-total", "mailq-count[active]", "mailq-count[deferred]", "mailq-count[hold]"]:
